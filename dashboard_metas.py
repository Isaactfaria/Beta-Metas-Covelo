...
def render_employee_list(employees, setor):
    ...
    <div style='
        ...
        align-items: center;
    '>
        <span>Equipe ({len(employees)} {'' if len(employees) == 1 else 'membros'})</span>
        <span>Total: {formatar_moeda(sum(emp['total'] for emp in employees))}</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    with container:
        ...
        for emp in employees:
            ...
            '>
                <div>
                    <span style='font-weight: 500;'>{emp['nome']}</span>
                    <span style='color: #7f8c8d; font-size: 0.9em; margin-left: 8px;'>{emp['cargo']}</span>
                </div>
                <div style='font-weight: 500; color: #27ae60;'>
                    {formatar_moeda(emp['total'])}
                </div>
            </div>
            """, unsafe_allow_html=True)
